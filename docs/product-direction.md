...
作品詳情一律使用多張 2D 圖片組成的畫廊呈現：由馬卡龍預設視角與多視角圖建立甜點印象，再以實際專案截圖和作者親自撰寫的介紹，讓訪客看見作品與創作者本人的關係。3D 不再是作品詳情的展示方向；首頁信件結語保留一個使用者主動載入的紫夜兔耳馬卡龍 3D viewer，作為走完整盒收藏後的特別收尾，不取代任何作品的 2D 畫廊。
...
8. **每種語言都有可直接分享的正式網址。** 中文、英文首頁與每個作品詳情都必須能在 GitHub Pages 直接開啟，並提供與該網址一致的搜尋與分享資訊；完整契約由 `docs/seo-routing.md` 持有。
...
- 作品詳情不載入或呈現馬卡龍 3D 模型；首頁信件結語可以在使用者明確按下按鈕後，載入紫夜兔耳馬卡龍 3D viewer。按下前不得請求 viewer runtime 或 GLB。
- 四個主要焦點專案的視覺層級最高，但不能破壞整盒一致性。
- 網站部署於 GitHub Pages；中文與英文使用分離的 clean URL，SEO、結構化資料與分享預覽由 `docs/seo-routing.md` 持有。
